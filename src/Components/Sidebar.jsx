...
const Sidebar = ({ activeSection, setActiveSection, theme, toggleTheme }) => {
  ...
  return (
    <>
      ...
      <div className={`lg:hidden fixed top-0 left-0 right-0 z-40 backdrop-blur-md border-b px-4 py-2.5 flex items-center justify-between transition-colors ${
        isLight ? 'bg-white/95 border-slate-200 text-slate-900 shadow-xs' : 'bg-[#0b0f17]/95 border-white/10 text-white'
      }`}>
        {/* Mobile Social Links (Visible on top bar when drawer is closed) */}
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={toggleTheme}
            className={`p-1.5 sm:p-2 rounded-xl border text-xs font-mono flex items-center gap-1 transition active:scale-95 ${
          ...
          <button
            ...
            className={`p-1.5 sm:p-2 rounded-xl border transition active:scale-95 ${
      ...
      {mobileOpen && (
        <div 
          className="lg:hidden fixed inset-0 z-50 bg-black/75 backdrop-blur-sm transition-opacity duration-300"
          onClick={() => setMobileOpen(false)}
        />
      )}

      {/* Sidebar Main Container (Smooth Mobile Drawer & Sticky Desktop Sidebar) */}
      <aside className={`
        ...
        ${isLight 
          ? 'bg-white/98 lg:bg-white/90 border-slate-200 text-slate-800 shadow-2xl' 
          : 'bg-[#0b0f17]/98 lg:bg-[#0b0f17]/85 border-white/10 text-slate-100 shadow-2xl'}
      ...
      `}>
        ...
        <div className={`relative rounded-2xl border overflow-hidden backdrop-blur-md transition-colors ${
          ...
          <div className={`relative h-24 sm:h-28 w-full overflow-hidden border-b ${
            ...
            {/* Top Right Mobile Controls (Close X button + Theme Toggle) inside drawer */}
            <div className="absolute top-2.5 right-2.5 flex items-center gap-2 z-10">
              <div className="font-mono text-[10px] text-emerald-300 bg-black/60 px-2 py-0.5 rounded-full border border-emerald-500/30 backdrop-blur-xs hidden sm:block">
                MERN & AI
              </div>
              ...
              <button
                onClick={() => setMobileOpen(false)}
                className="lg:hidden p-1.5 rounded-full bg-slate-950/80 hover:bg-slate-900 border border-white/20 text-slate-300 hover:text-white transition shadow-lg"
          ...
          <div className="p-3.5 sm:p-4 pt-0">
            ...
            <div className="flex flex-col items-center text-center">
              <div className="relative -mt-11 sm:-mt-13 mb-2.5 group">
                <div className={`relative w-22 h-22 sm:w-26 sm:h-26 rounded-full overflow-hidden p-0.5 ${
            ...
            <div className={`mt-3.5 pt-3 border-t flex flex-col gap-2.5 ${
